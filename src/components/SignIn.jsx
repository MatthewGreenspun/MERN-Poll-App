...
export default function SignIn(props) {
    ...
    async function handleNewUserSubmit(e) {
        ...
        if(username.length < 5) {
            ...
            try {
                const newUserId = await axios.post("http://localhost:5000/users/add", {username});
                console.log(newUserId);
                const newUserData = await axios.get(`http://localhost:5000/users/${newUserId.data.id}`);
    ...
    return (
        <div className="sign-in-page">
            <form className="sign-in-form">
                ...
                <button type="submit" id="sign-in-btn" onClick={(e) => handleIdSubmit(e)}>Sign In</button>
                <br />
                <p className="create-new-account">or create a new account below</p>
                <input type="text" className="form-input" id="id-input" value={username} onChange={(e) => setUsername(e.target.value)} />
                <p className="error-txt">{usernameError}</p>
                <button type="submit" id="sign-in-btn" onClick={(e) => handleNewUserSubmit(e)}>Create Account</button>
            </form>
        </div>
    )
}